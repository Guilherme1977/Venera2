...
const TicketCard: React.FC<{ contractLink?: string; lotteryNumber?: number }> = ({ contractLink, lotteryNumber }) => {
  ...
  return (
    <Wrapper>
      <div>
        <Button disabled={ticketsLength === 0} onClick={onPresentMyTickets} fullWidth className="btn_yellow">
          {TranslateString(999, 'View your tickets')}
        </Button>
      </div>
      <div>
        <ExternalLinkWrap  className="blue_font_txt" href={contractLink}>{TranslateString(999, 'View on BscScan')}</ExternalLinkWrap>
      </div>
    </Wrapper>
  )
}
